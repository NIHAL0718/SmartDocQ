...
class VectorStore:
    """Vector store service for document chunks using ChromaDB."""
    ...
    def __init__(self, persist_directory: str = None):
        ...
        self.chroma_collection = self.chroma_client.get_or_create_collection("document_chunks")
    
    def store_document_chunks(self, chunks: List[DocumentChunk]) -> Dict[str, Any]:
    ...
    def search_similar_chunks(self, query: str, doc_id: Optional[str] = None, limit: int = 5) -> List[SourceChunk]:
...
# For backward compatibility, keep the global instances and functions
...
def store_document_chunks(chunks: List[DocumentChunk]) -> Dict[str, Any]:
    """Store document chunks in the vector database (legacy function).
    ...
    Returns:
        Dictionary with storage results
    """
    vector_store = VectorStore(PERSISTENCE_DIRECTORY)
    return vector_store.store_document_chunks(chunks)


def search_similar_chunks(query: str, doc_id: Optional[str] = None, limit: int = 5) -> List[SourceChunk]:
    """Search for document chunks similar to the query (legacy function).
    ...
    """
    vector_store = VectorStore(PERSISTENCE_DIRECTORY)
    return vector_store.search_similar_chunks(query, doc_id, limit)


def get_document_chunks(doc_id: str) -> List[DocumentChunk]:
    """Get all chunks for a specific document (legacy function).
...
def delete_document_chunks(doc_id: str) -> Dict[str, Any]:
    """Delete all chunks for a specific document (legacy function).
    ...
    """
    vector_store = VectorStore(PERSISTENCE_DIRECTORY)
    return vector_store.delete_document_chunks(doc_id)


def get_collection_stats() -> Dict[str, Any]:
    """Get statistics about the vector database collection (legacy function).
